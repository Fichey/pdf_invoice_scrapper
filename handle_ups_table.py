...
def is_there_reference_number(text: str) -> bool:
    ...
    return bool(re.search(r"\d{9}|[()_]", text))
...
def handle_fedex_table(table: List[List[str]], numer_faktury) -> Optional[Dict]:
    ...
    try:
        ...
        data_wysylki = datetime.strptime(rows[0][0].split('\n')[0].split(' ')[1], "%d/%m/%Y").date().isoformat() if rows[0][0].split('\n')[0].split(' ')[1] else None
        ...
        czas_match = re.search(r"\b\d{2}/\d{2}/\d{4} \d{2}:\d{2}\b", rows[2][0])
        czas_odebrania = datetime.strptime(czas_match.group(0), "%d/%m/%Y %H:%M").isoformat() if czas_match else None
