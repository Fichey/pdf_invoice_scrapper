...
# --- Ekstrakcja tabel z pdfplumber (kilka strategii) ---
...
def process_pdf(path: str) -> Dict[str, Any]:
    import pdfplumber
    result: Dict[str, Any] = {"plik": path, "strony": []}
    with pdfplumber.open(path) as pdf:
        for pi, page in enumerate(pdf.pages, start=1):
            rec_page: Dict[str, Any] = {"strona": pi, "tabele": []}
            for ti, table in enumerate(try_extract_tables(page), start=1):
                if detect_fedex_like_table(table):
                    ...
                    parsed = handle_generic_table(table)
                parsed["indeks"] = ti
                rec_page["tabele"].append(parsed)
            result["strony"].append(rec_page)
    return result
